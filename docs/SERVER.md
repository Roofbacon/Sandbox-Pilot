...
| `sandbox_winget` | Run WinGet search/show/install/upgrade/uninstall/list inside the Sandbox, optionally bootstrapping WinGet first and adding automation-friendly agreement/noninteractive/silent flags. |
...
| `sandbox_watch_start` / `sandbox_watch_stop` | Start/stop a background runspace that diffs top-level windows, the foreground window, and process names (~every `intervalMs`) into a capped, cursor-indexed event buffer. |
| `sandbox_watch_poll` / `sandbox_wait_for_event` | Drain new watcher events since the last poll, or block until a new event matching `type`/`contains` occurs (or timeout). `sandbox_test_install_command` auto-starts the watcher and returns `windowsDuringRun`. |
