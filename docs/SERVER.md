...
| `sandbox_test_phishing_url` | Analyze a direct URL or whole email for phishing indicators: extract links, score static email/URL signals, run DNS/TLS/HTTP/redirect checks inside the Sandbox, open the selected target in Edge InPrivate for foreground evidence, and write `report.md` plus `results.json` under `artifacts/phishing-url-tests/<runId>`. |
